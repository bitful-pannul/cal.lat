...
wit_bindgen::generate!({
    path: "target/wit",
    world: "callat-test-template-dot-os-v0",
...
fn handle_message(our: &Address) -> anyhow::Result<()> {
    ...
    print_to_terminal(0, "callat_test: a");
    assert!(node_names.len() == 1);

    let our_callat_address = Address {
        node: our.node.clone(),
        process: ProcessId::new(Some("callat"), "callat", "template.os"),
    };

    // Send
    print_to_terminal(0, "callat_test: b");
    let response = Request::new()
        .target(our_callat_address)
...
fn init(our: Address) {
    ...
    loop {
        match handle_message(&our) {
            Ok(()) => {}
            Err(e) => {
                print_to_terminal(0, format!("callat_test: error: {e:?}").as_str());
